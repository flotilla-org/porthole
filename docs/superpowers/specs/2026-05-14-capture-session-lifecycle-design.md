...
The daemon registry owns session state. V1 needs only three externally visible
states:
...
Closed sessions are removed from the registry. A later lookup returns the same
unknown-session behavior as any other missing id. We can add tombstones later if
clients need post-close diagnostics.
...
`CaptureSessionResponse` exposes the actual registry state, including `starting`
or `failed` if future discovery can observe them.
...
it can start as `ready`. Its background task should mark the session `failed`
when `next_frame()` returns an error and should stop quietly on normal end.

Dropping a `CaptureSession` must abort any owned-frame task. Removing a session
must therefore stop both the publisher path, by dropping its native capture
handle, and the owned-frame path, by aborting its task and dropping the capture
future.
