...
- `recently_active_surface_ids` — in slice A, this is always empty. Populating MRU tracking requires focus-change observation (either polling or native AX observers); both are deferred. The field is present in the response shape so callers can start coding against it, but they should expect empty values until the events slice lands. See Known Limitations.
