...
$approvalCount = 0
do {
    if (Test-Path -LiteralPath (Join-Path $RunDirectory 'agent-desktop-result.json')) { Get-Content -LiteralPath (Join-Path $RunDirectory 'agent-desktop-result.json'); exit 0 }
...
Write-Output "Approval window ended: $approvalCount requests approved; agent result not yet present. Inspect the agent's progress and rerun approve-proof.ps1 with the same RunDirectory to continue waiting."
