...
if (Test-Path -LiteralPath $statePath) {
    ...
    if (-not (Test-Path $entryPath)) { throw 'Existing run has no verified agent entry; inspect it rather than replacing it' }
...
if ($sessionId -eq 0 -or -not (Get-Process explorer | Where-Object SessionId -eq $sessionId)) { throw 'Start from the existing GUI login' }
...
$config | ConvertTo-Json | Set-Content -Encoding UTF8 (Join-Path $RunDirectory 'config.json')
...
"@
$task | Set-Content -Encoding UTF8 (Join-Path $Workspace 'request.md')
...
try {
    $ready = $false
    for ($i=0; $i -lt 30; $i++) {
        try { $info = Invoke-PortholeJson GET '/info'; if ($info.Status -eq 200) { $ready=$true; break } } catch { Start-Sleep -Milliseconds 200 }
    }
    ...
    if ($launch.Status -lt 200 -or $launch.Status -ge 300) {
        ...
        $launch.Body | ConvertTo-Json -Depth 8 | Set-Content -Encoding UTF8 (Join-Path $RunDirectory 'launch-error.json')
        throw "Console launch failed with HTTP $($launch.Status); inspect launch-error.json and any test-owned process before retrying"
    }
    $launch.Body | ConvertTo-Json -Depth 8 | Set-Content -Encoding UTF8 (Join-Path $RunDirectory 'terminal-launch.json')
    Write-Output "STARTED: $Server/$Session; identity $($identity.agent_id); surface $($launch.Body.surface_id); Session $sessionId"
} catch {
    if ($identity) { Invoke-PortholeJson POST "/agent-identities/$($identity.agent_id)/revoke" @{} | Out-Null }
    throw
} finally {
