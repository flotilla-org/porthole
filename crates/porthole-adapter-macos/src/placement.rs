...
use porthole_core::{ErrorCode, PortholeError, display::Rect, surface::SurfaceInfo};
...
pub async fn place_surface(adapter: &MacOsAdapter, surface: &SurfaceInfo, rect: Rect) -> Result<(), PortholeError> {
    ...
    crate::close_focus::with_ax_window_by_cg_id(pid, cg, |raw| {
        let (err_pos, err_size) =
            unsafe { AxElement::with_borrowed(raw, |elem| (elem.set_position(rect.x, rect.y), elem.set_size(rect.w, rect.h))) };
