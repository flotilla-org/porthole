//! `porthole install` / `porthole uninstall` — place the bundle and CLI.
//!
...
//!
//! Registering the background daemon is no longer install's job: launching the
//! bundled helper (a login item) registers portholed as its own launchd
//! LaunchAgent via `SMAppService.agent` (see `apps/macos`). That is what lets
//! the daemon vend the attach MachService. Install therefore just places the
...
fn do_install(opts: InstallOptions) -> Result<(), InstallError> {
    ...
    println!();
    println!("done. next:");
    println!("  1. open {} once — the helper registers the background", dst_bundle.display());
    println!("     daemon with launchd (SMAppService) and starts it now and at login.");
    println!("     You may need to approve it under System Settings > General > Login Items.");
    println!("  2. run `porthole onboard` to grant TCC permissions.");
    Ok(())
}
